...
export const SessionProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const setCurrentSessionAndPersist = useCallback(
  ...
  const updateRow = useCallback(
    (rowIndex: number, updater: (row: Unit[]) => Unit[]) => {
  ...
  const duplicateRow = useCallback(
    (rowIndex: number) => {
      updateSession((session) => {
        ...
        const duplicatedRow = rowToDuplicate.map((unit) =>
          unitService.duplicateUnit(unit)
        );
  ...
  const addUnit = useCallback(
    (rowIndex: number, addCount: number) => {
      updateRow(rowIndex, (row) => {
        const newUnits = Array.from({ length: addCount }, () =>
  ...
  const updateUnit = useCallback(
    (rowIndex: number, unitId: string, updates: Partial<Unit>) => {
      updateRow(rowIndex, (row) =>
        row.map((unit) =>
  ...
  const updateMultipleUnits = useCallback(
    (rowIndex: number, updatedUnits: Unit[]) => {
      updateSession((session) => {
        const newRows = [...session.rows];
  ...
  const deleteUnits = useCallback(
    (rowIndex: number, unitIds: string[]) => {
      updateRow(rowIndex, (row) =>
        row.filter((unit) => !unitIds.includes(unit.id))
      );
    },
    [updateRow]
  );
  ...
  const duplicateUnits = useCallback(
    (rowIndex: number, unitIds: string[], count: number) => {
      updateRow(rowIndex, (row) => {
        const unitsToDuplicate = unitIds
          .map((unitId) => row.find((unit) => unit.id === unitId))
          .filter(Boolean);
        const newUnits = Array.from({ length: count }, () =>
          unitsToDuplicate.map((unit) =>
            unitService.duplicateUnit(unit as Unit)
          )
        ).flat();
        return [...row, ...newUnits];
      });
