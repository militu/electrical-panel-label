...
const RowContainer: React.FC<RowContainerProps> = ({rowIndex, units, onUpdate}) => {
    const t = useTranslations('RowContainer');
    const {addUnit, updateUnit, deleteUnits, deleteUnit, duplicateUnits, updateRow, updateMultipleUnits} = useSession();
    ...
    const [dragAndDropRef, dragAndDropUnits, setDragAndDropUnits] = useDragAndDrop<HTMLDivElement, Unit>(units, {
        group: `row-${rowIndex}`,
    ...
    const toggleUnitSelection = useCallback((unitId: string, isSelected: boolean) => {
    ...
    const handleEditSelected = useCallback(() => {
        const selectedUnitObjects = dragAndDropUnits.filter(unit => selectedUnits.includes(unit.id));
                ...
                <div className="flex flex-col md:flex-row gap-2 w-full tour-unit-management-buttons">
                    <div className="flex flex-row gap-1 w-full md:w-auto">
                        <Input
                            ...
                            className="w-20"
                        />
                        <Button onClick={handleAddUnits} className="flex-grow md:flex-grow-0">
                            {t('addUnits')}
                        </Button>
                    </div>
                    <Button onClick={handleSelectAll} variant="secondary" className="w-full md:w-auto">
                        {t('selectAll')}
                    </Button>
                    <Button onClick={handleDeselectAll} variant="secondary" className="w-full md:w-auto">
                        {t('deselectAll')}
                    </Button>
                </div>
                        ...
                        <AlertDialog open={showDeleteAlert} onOpenChange={setShowDeleteAlert}>
                            ...
                            <AlertDialogContent>
                                <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>{t('deleteSelectedConfirmDescription')}</AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                    <AlertDialogCancel>{t('cancel')}</AlertDialogCancel>
                                    <AlertDialogAction onClick={confirmDeleteSelected}>{t('delete')}</AlertDialogAction>
                                </AlertDialogFooter>
                            </AlertDialogContent>
                        </AlertDialog>
                    </div>
