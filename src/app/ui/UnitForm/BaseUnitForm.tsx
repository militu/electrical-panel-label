...
const BaseUnitForm: React.FC<BaseUnitFormProps> = ({
    ...
    React.useEffect(() => {
        const checkScroll = () => {
            if (scrollContainerRef.current) {
                ...
                setShowScrollIndicator(hasScrollableContent && !isNearBottom && !hasScrolled);
            }
        };
        ...
        return () => {
            ...
            if (scrollContainer) {
                ...
                scrollContainer.removeEventListener('scroll', () => setHasScrolled(true));
            }
            clearTimeout(timer);
        };
    }, [hasScrolled]);
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col h-full relative">
            ...
            {showScrollIndicator && (
                <div
                    className="absolute bottom-32 left-1/2 transform -translate-x-1/2 w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center animate-bounce">
                    <svg className="w-4 h-4 text-gray-600" fill="none" strokeLinecap="round" strokeLinejoin="round"
            ...
            <div
                className="mt-4 flex flex-col sm:flex-row items-center justify-end gap-2 pt-4 border-t sticky bottom-0 bg-background p-4">
