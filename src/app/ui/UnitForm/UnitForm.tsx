...
const UnitForm: React.FC<UnitFormProps> = ({
  ...
  const updateUnitSvg = useCallback(
    async (unitToUpdate: Unit) => {
    ...
    <div className="flex flex-col lg:flex-row w-full h-full gap-6">
      {!isSmallScreen && (
        <div className="lg:w-1/2 xl:w-2/5 flex flex-col">
          ...
          <div className="flex-grow flex items-center justify-center bg-white rounded-lg shadow-md p-4 overflow-hidden">
            {unitSvg && (
              <div
      ...
      <div
        className={`${
          isSmallScreen ? "w-full" : "lg:w-1/2 xl:w-3/5"
        } flex flex-col`}
      >
