...
const PanelLayout: React.FC = () => {
  ...
  const [latestModifiedRow, setLatestModifiedRow] = useState<number | null>(
    null
  );
  ...
  useEffect(() => {
    if (latestModifiedRow !== null && accordionRef.current) {
      const newRowElement = accordionRef.current.querySelector(
        `[data-row-id="row-${latestModifiedRow}"]`
      );
      if (newRowElement) {
        setTimeout(() => {
          const elementPosition = newRowElement.getBoundingClientRect().top;
          const offsetPosition =
            elementPosition + window.scrollY - SCROLL_HEADROOM;
  ...
  return (
    <div className={`rounded-xl shadow-lg p-4 md:p-6 mb-8 tour-panel-layout`}>
      ...
      <Accordion
        ...
        ref={accordionRef}
      >
        <Suspense fallback={<SkeletonLoader />}>{rowContainers}</Suspense>
      </Accordion>
