...
const SessionManagement: React.FC = () => {
    const {sessions, createSession, setCurrentSessionByID, getSessionNameById, deleteSession} = useSession();
            ...
            <div className="flex flex-col md:flex-row tour-sessions gap-4">
                <Dialog>
                    <DialogTrigger asChild>
                        <Button variant="outline" className="w-full md:w-auto">{t('buttons.new.button')}</Button>
                    </DialogTrigger>
                    <DialogContent>
                        <DialogHeader>
                        ...
                        <div className="flex max-w-sm items-center space-x-2">
                            ...
                            <DialogFooter className="md:justify-start">
                                <DialogClose asChild>
                                    <Button
                                        onClick={handleCreateSession}>{t("buttons.new.dialog.submit_button")}</Button>
                                </DialogClose>
                            </DialogFooter>
                        </div>
                    </DialogContent>
                ...
                <Dialog>
                    <DialogTrigger asChild>
                        <Button variant="outline" className="w-full md:w-auto">{t('buttons.load.button')}</Button>
                    </DialogTrigger>
                    <DialogContent>
                        <DialogHeader>
                        ...
                        <div className="flex max-w-sm items-center space-x-2">
                            <Select onValueChange={setSelectedSessionId}>
                                <SelectTrigger>
                                    <SelectValue placeholder={t('buttons.load.dialog.select_placeholder')}/>
                                </SelectTrigger>
                                <SelectContent>
                                    {sessions.map((session) => (
                                        <SelectItem key={session.id} value={session.id}>
                                            {`${session.name} (${new Date(session.lastModified).toLocaleString()})`}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                            <DialogFooter className="md:justify-start">
                                <DialogClose asChild>
                                    <Button
                                        onClick={handleLoadSession}>{t('buttons.load.dialog.submit_button')}</Button>
                                </DialogClose>
                            </DialogFooter>
                        </div>
                    </DialogContent>
                ...
                <Dialog>
                    <DialogTrigger asChild>
                        <Button variant="outline" className="w-full md:w-auto">{t('buttons.delete.button')}</Button>
                    </DialogTrigger>
                    <DialogContent>
                        <DialogHeader>
                        ...
                        <div className="flex max-w-sm items-center space-x-2">
                            <Select onValueChange={setSelectedSessionId}>
                                <SelectTrigger>
                                    <SelectValue placeholder={t('buttons.delete.dialog.select_placeholder')}/>
                                </SelectTrigger>
                                <SelectContent>
                                    {sessions.map((session) => (
                                        <SelectItem key={session.id} value={session.id}>
                            ...
                            <DialogFooter className="md:justify-start">
                                <Button
                                    onClick={handleDeleteSession}>{t('buttons.delete.dialog.submit_button')}</Button>
                            </DialogFooter>
                        </div>
                    </DialogContent>
                </Dialog>
