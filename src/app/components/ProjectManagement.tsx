...
const ProjectManagement: React.FC = () => {
    ...
    const exportProject = () => {
        if (currentSession) {
            ...
            saveAs(blob, `${currentSession.name.replace(/\s+/g, '_')}_${new Date().toISOString()}.json`);
        } else {
            toast({
    ...
    const handleImport = (file: File) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                const isValidFormat = importedData.name && Array.isArray(importedData.rows) && importedData.globalSettings;
                if (isValidFormat) {
    ...
    return (
        <div className={`flex flex-col md:flex-row gap-4 w-full md:w-auto tour-project-management`}>
            <Button onClick={exportProject} className="w-full md:w-auto">{t('exportProject')}</Button>
